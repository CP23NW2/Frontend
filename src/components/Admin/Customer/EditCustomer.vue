      ...
      <div class="w-full h-screen mt-4 bg-gray-200 md:mt-12">
        <div class="flex items-center justify-center p-4 py-10 md:py-20 md:p-10">
          <div id="printMe" class="w-full h-auto mt-4 bg-white rounded-md shadow md:mt-0">
            ...
            <form @submit.prevent="" ref="customerForm" class="flex p-4 py-5 md:px-12">
...
export default{
  ...
  methods: {
    ...
    updateCustomer() {
      const url = `http://localhost:3000/customers/${this.customerTel}`;
      axios.put(url, this.customerData)
        .then((response) => {
  ...
  computed: {
    formattedTel: {
      get() {
        return this.customerData.customerTel ? '0' + this.customerData.customerTel : '0';
      },
      set(value) {
